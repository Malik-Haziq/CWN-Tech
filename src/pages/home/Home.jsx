...
export function Home() {
  const headingList = ['Web Development','Software Development','UI & UX Design','SEO (Search Engine Optimization)','Marketing Services','App Development',]
  const services = [
    {
...
function ServicesSection({ headingList, heading, desc, list }) {
  return (
    <div className="flex gap-16 pr-24 shadow-3xl relative translate-x-[2%] w-[98%] rounded-md">
      <aside className="bg-main-shade min-w-fit py-6 px-2 shadow-2xl text-white font-medium text-xl flex flex-col justify-between items-center rounded-md -translate-y-4 -translate-x-7">
        {headingList.map(el=>{
      ...
      <div className="my-10">
        <h3 className="h3 mb-12 text-sub-heading">{heading}</h3>
        <p className="text-sub-para text-xl leading-8 mb-20">{desc}</p>
        <ul className="grid grid-cols-2 text-xl gap-8 list-disc list-inside text-sub-heading">
          {list.map((li) => {
            <li>{li}</li>;
          })}
